...
var _ = XDescribe("Key Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind Key")
			err := k8sClient.Get(ctx, typeNamespacedName, key)
			if err != nil && errors.IsNotFound(err) {
				resource := &resticv1.Key{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					Spec: resticv1.KeySpec{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Key")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &controller.KeyReconciler{
				Client: k8sClient,
