...
func getPodContainrLogs(ctx context.Context, config *rest.Config, pod *resticv1.Pod) (string, error) {
	clientset, err := kubernetes.NewForConfig(config)
	if err != nil {
		return "", err
	}

	req := clientset.CoreV1().Pods(pod.Namespace).GetLogs(pod.Name, &resticv1.PodLogOptions{})
